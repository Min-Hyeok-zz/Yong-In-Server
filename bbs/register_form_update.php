...
if (trim($mb_sid == "") || trim($mb_password == "") || trim($mb_password_re == "") || trim($mb_name == "") || trim($mb_email == "") || trim($mb_tel == "")){
    alert_close("필수 입력값 중에 빈 값이 있습니다.");
}

$check1         = $_POST['check1'];
$check2         = $_POST['check2'];
$mb_sid         = trim($_POST['mb_sid']);
$mb_password    = trim($_POST['mb_password']);
$mb_password_re = trim($_POST['mb_password_re']);
$mb_name        = trim($_POST['mb_name']);
$mb_email       = trim($_POST['mb_email'][0].$_POST['mb_email'][1].$_POST['mb_email'][2]);
$mb_tel         = trim($_POST['mb_tel'][0].$_POST['mb_tel'][1].$_POST['mb_tel'][2]);
$mb_birth       = isset($_POST['mb_birth']) ? trim($_POST['mb_birth'][0].$_POST['mb_birth'][1].$_POST['mb_birth'][2]) : "";
...
if ($w == '' || $w == 'u') {
    ...
    if ($msg = empty_mb_id($mb_id))        echo "<script>alert('{$msg}');return false;</script>"; // alert($msg, $url, $error, $post);
    if ($msg = valid_mb_id($mb_id))        echo "<script>alert('{$msg}');return false;</script>";
    if ($msg = count_mb_id($mb_id))        echo "<script>alert('{$msg}');return false;</script>";

    // 이름, 닉네임에 utf-8 이외의 문자가 포함됐다면 오류
    // 서버환경에 따라 정상적으로 체크되지 않을 수 있음.
    if (!isset($_POST['sid_value']))
        echo "<script>alert('학번 중복체크를 하지 않으셨습니다.');return false;</script>";
    ...
    if ($w == '' && !$mb_password)
        echo "<script>alert('비밀번호가 넘어오지 않았습니다.');return false;</script>";

    if($w == '' && $mb_password != $mb_password_re)
        echo "<script>alert('비밀번호와 비밀번호 확인이 다릅니다.');return false;</script>";

    if($check1 != "on" || $check2 != "on")
        echo "<script>alert('약관에 동의 해 주세요');return false;</script>";
    ...
    if (!preg_match("/([a-zA-Z0-9]+@[a-zA-Z0-9]+\.[a-zA-Z0-9]+)/", $mb_email)) echo "<script>alert('이메일 형식이 올바르지 않습니다.');return false;</script>";
    ...
    if ($w=='') {
        if ($msg = exist_mb_id($mb_id))     alert($msg);
            if ($db->query("SELECT * FROM g5_member where mb_id='{$mb_id}'")->rowCount() > 0) alert("중복된 아이디 입니다.","",true,true);
            if ($db->query("SELECT * FROM g5_member where mb_email='{$mb_email}'")->rowCount() > 0) alert("중복된 이메일 입니다.","",true,true);
            if ($db->query("SELECT * FROM g5_member where mb_sid='{$mb_sid}'")->rowCount() > 0) alert("중복된 학번 입니다.","",true,true);
            if ($db->query("SELECT * FROM g5_member where mb_tel='{$mb_tel}'")->rowCount() > 0) alert("중복된 전화번호 입니다.","",true,true);
