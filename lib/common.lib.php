...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    // if ($cur_page > 1)
        $str .= '<a href="'.$url.'1'.$add.'"><i class="fas fa-angle-double-left"></i></a>'.PHP_EOL;
    ...
    if ($start_page > 1)
        $str .= '<a href="'.$url.($start_page-1).$add.'"><i class="fas fa-angle-double-left"></i></a>'.PHP_EOL;

    // if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            $active = '';
            if ($cur_page == $k) $active = " active";
            $str .= '<a href="'.$url.$k.$add.'" class="page-btn'.$active.'">'.$k.'</a>'.PHP_EOL;
        }
    // }

    if ($total_page > $end_page)
        $str .= '<a href="'.$url.($end_page+1).$add.'"><i class="fas fa-angle-right"></i></a>'.PHP_EOL;
    // if ($cur_page < $total_page)
    // if ($cur_page < $total_page)
        $str .= '<a href="'.$url.$total_page.$add.'"><i class="fas fa-angle-double-right"></i></a>'.PHP_EOL;

    if ($total_page > 0)
        return "<div class=\"pageing\">{$str}</div>";
    else
        return "";
}
